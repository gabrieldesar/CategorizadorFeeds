...
function recuperarListaPostsPolitica(postsSpinner) {
	  var accessToken = FB.getAuthResponse()['accessToken'];
      var accessTokenString = JSON.stringify({"accessToken": accessToken});
      
      jQuery.ajax({
          type: "POST",
          url: rootURL + '/politica',
          data: accessTokenString,
          contentType: "application/json",
          error: function (xhr, status) {
                  console.log("Ocorreu um erro ao obter a lista de posts sobre política: " + status + '.');
              },
          success: function (data, msg) {
              console.log("Lista de posts sobre política recuperada com sucesso.");
              renderizarListaPostsPolitica(data, postsSpinner);
          }
      });
}
  
function renderizarListaPostsPolitica(data, postsSpinner) {
...
jQuery(window).load(function() {
	...
	FB.getLoginStatus(function(response) {
	  if (response.status === 'connected') {
		recuperarListaPostsPolitica(postsSpinner);
	  } else {
