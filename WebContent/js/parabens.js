...
function recuperarListaPostsParabens(postsSpinner) {
	  var accessToken = FB.getAuthResponse()['accessToken'];
      var accessTokenString = JSON.stringify({"accessToken": accessToken});
      
      jQuery.ajax({
          type: "POST",
          url: rootURL + '/parabens',
          data: accessTokenString,
          contentType: "application/json",
          error: function (xhr, status) {
                  console.log("Ocorreu um erro ao obter a lista de posts sobre parabens: " + status + '.');
              },
          success: function (data, msg) {
              console.log("Lista de posts sobre parabens recuperada com sucesso.");
              renderizarListaPostsParabens(data, postsSpinner);
          }
      });
}
  
function renderizarListaPostsParabens(data, postsSpinner) {
...
jQuery(window).load(function() {
	...
	FB.getLoginStatus(function(response) {
	  if (response.status === 'connected') {
		recuperarListaPostsParabens(postsSpinner);
	  } else {
